 ...
 */
  ...
  */
  async openSettings() {
    ...
    await this.firestore.collection('eCitizens/').doc(user.displayName).ref.get().then((doc) => {
      if (doc.exists) {
  ...
  */
  ...
  async NICStatus() {
    ...
    await this.firestore.collection('/eApplications/').doc('NICApplication').ref.get().then((doc) => {
      if (doc.exists) {
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: async (alertData) => {
            var user = firebase.auth().currentUser;
            user.updatePassword(alertData.password).then(async (res) => {
    ...
    const alert = await this.alertController.create({
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: async (alertData) => {
            var user = firebase.auth().currentUser;
            user.updateEmail(alertData.email).then(async (res) => {
              const toast = await this.toastController.create({
