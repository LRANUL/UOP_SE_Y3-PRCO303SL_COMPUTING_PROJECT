/**
 * CONTAINS REMOTECONFIG CLASS CODE FOR SECRETARY FUNCTIONALITY
 */
...
export class RemoteConfigService {
  ...
  private async ngOnInit() {
    ...
    this.remoteConfig.defaultConfig = {
      ...
      secretary_system_maintenance: "false",
    };
  }
  /**
   * Method for mantenance locks
   * @returns boolean
   */
  ...
  async secretarykMaintenanceLockCheck() {
    if (this.remoteConfig) {
      const secretaryMaintenanceLock =
        (await this.secretaryMaintenanceLock()) || "false";
      if (secretaryMaintenanceLock == "true") {
        return true;
      }
      return false;
    }
  ...
  private async secretaryMaintenanceLock() {
    return new Promise<string>((resolve, reject) => {
      this.remoteConfig
        .fetchAndActivate()
        .then(() => {
          this.remoteConfig
            .getString({ key: "secretary_system_maintenance" })
            .then((data) => resolve(data))
            .catch((err) => reject(err));
        })
        .catch((err) => reject(err));
